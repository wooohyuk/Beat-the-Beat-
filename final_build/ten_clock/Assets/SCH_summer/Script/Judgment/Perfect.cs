...
public class Perfect : MonoBehaviour 
{
    ...
    void OnCollisionStay2D(Collision2D col)
    {
        ...
        {
            GameObject.Find("DeterChecker").GetComponent<Judgment_Manage>().noteNumCheckP = 1;
            noteCheck = col.gameObject;
        }

		if (col.gameObject.tag == "RightN")
        {
            GameObject.Find("DeterChecker").GetComponent<Judgment_Manage>().noteNumCheckP = 2;
            noteCheck = col.gameObject;
        }

        if (col.gameObject.tag == "TapN")
        {
            GameObject.Find("DeterChecker").GetComponent<Judgment_Manage>().noteNumCheckP = 3;
            noteCheck = col.gameObject;
        }
    }
