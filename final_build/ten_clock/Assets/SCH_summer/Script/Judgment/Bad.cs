...
public class Bad : MonoBehaviour
{
    ...
    void OnCollisionStay2D(Collision2D col)
    {
        ...
        {
            GameObject.Find("DeterChecker").GetComponent<Judgment_Manage>().noteNumCheckB = 1;
            noteCheck = col.gameObject;
        }

        if (col.gameObject.tag == "RightN")
        {
            GameObject.Find("DeterChecker").GetComponent<Judgment_Manage>().noteNumCheckB = 2;
            noteCheck = col.gameObject;
        }

        if (col.gameObject.tag == "TapN")
        {
            GameObject.Find("DeterChecker").GetComponent<Judgment_Manage>().noteNumCheckB = 3;
            noteCheck = col.gameObject;
        }
    }
